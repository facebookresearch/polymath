...
from inference.chat_completion import ChatCompletion, Role, Message
...
1. A Python or Prolog code snippet that includes domain-specific annotations or constraints.
2. An error output from a formal verification or logic programming tool (such as CLP(FD), Z3, etc.).
...
Then, **generate a prompt** that could be given to another LLM to automatically repair the faulty code.

The prompt you generate should:
- Point out the precise faulty expression or construct.
- Suggest the correct replacement.
- Clearly tell the LLM *not* to rewrite the whole code, but only to **replace the faulty part**.
- Be minimal, precise, and instructive.

Do not try to solve the problem yourself — just explain and help generate the fixing prompt.
...
solve :-
    ...
    % Constraints Section
    % Clue 1: The person whose mother's name is Holly is Arnold.
    (Holly #= 1 #<=> Arnold #= 1),
    (Holly #= 2 #<=> Arnold #= 2),
    ...
    write(Vars), nl.
## Error :
ERROR: /tmp/tmpraj6b6be.pl:16:15: Syntax error: Operator expected
ERROR: -g solve: catch/3: Unknown procedure: solve/0
    
##Thinking:
The error message Syntax error: Operator expected on line 16 is caused by the operator #<=>, which is not recognized by the CLP(FD) library.
In this domain, the correct operator for bidirectional logical implication is #<==>, not #<=>.

## Output :
In the following Prolog code, replace all occurrences of #<=> with the correct operator #<==>, which is the valid bidirectional implication supported by the clpfd library. Do not change anything else in the code.
...
"""
    ...
    async def __generate_and_verify_constraints(
        ...
        while True:
            ...
            if re.match(r"\s*solve",constraints):
                python_code: str =f"""
...
"""
            ...
            solver_constraints = python_code
...
{self.__result_trace.solver_constraints}
                    ...
                    finish_reason, ai_response = await self.__client.create(Message(Role.USER,ERROR_MESSAGE.format(python_code,stderr)))
            ...
            variables = LogicAgent.__extract_variables(python_code)
            nb_categories = LogicAgent.__extract_num(python_code)
            solver_outcome, output = self.__engine_strategy.parse_solver_output(
                    solver_exit_code, (stdout, variables, nb_categories),stderr
            )
            print("prolog output")
            print(stdout)
            print("\n\n")
